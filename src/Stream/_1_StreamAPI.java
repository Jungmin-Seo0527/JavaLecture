/*
    ...
    * 스트림 API
    ...
    즉, 데이터베이스의 쿼리와 같이 정형화된 처리 패턴을 가지지 못했기에 데이터마다 다른 방법으로 접근해야 했다.

    이러한 문제점을 극복하기 위해서 Java SE 8부터 스트림(Stream)API 를 도입한다.
    ...
    * 스트림 API의 특징
    ...
    1) 스트림은 외부 반복을 통해 작업하는 컬렉션과는 달리 내부 반복(internal iterator)을 통해 작업을 수행한다.
